...
📌 Giới thiệu
Dự án thực hành kết nối cơ sở dữ liệu MySQL bằng thư viện PDO (PHP Data Objects). Hệ thống chuyển đổi việc lưu trữ danh mục từ mảng/biến tạm thời sang lưu trữ vĩnh viễn trên CSDL. Cung cấp đầy đủ tính năng CRUD (Thêm, Đọc, Sửa, Xóa).

📂 Hướng dẫn cài đặt CSDL
Bật MySQL trên XAMPP.
Truy cập http://localhost/phpmyadmin/.
Import file schema.sql đính kèm trong dự án, hoặc sao chép mã SQL chạy trực tiếp.
Lệnh kiểm tra dữ liệu thành công: SELECT COUNT(*) FROM categories; (Kết quả trả về >= 3).
Thông tin cấu hình (tại config.php):
Host: 127.0.0.1
